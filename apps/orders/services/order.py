...
class OrderService:
    ...
    def cancel_order(order: Order, user) -> None:
        ...
        if order.status == 'paid':
            for item in order.items.select_related('product', 'fulfillment_warehouse'):
                if item.product and item.fulfillment_warehouse:
                    InventoryService.return_stock(
                        ...
                        user=user,
                        order=order,
                        warehouse=item.fulfillment_warehouse
                    )
        ...
        OrderStatusLog.objects.create(
            order=order, previous_status=old_status, new_status='cancelled', created_by=user
        )
    ...
    def mark_as_paid(order: Order) -> None:
        ...
        OrderStatusLog.objects.create(order=order, previous_status=old_status, new_status='paid')
    ...
    def update_status(order: Order, new_status: str, user=None) -> None:
        ...
        OrderStatusLog.objects.create(
            order=order, previous_status=old_status, new_status=new_status, created_by=user
        )
